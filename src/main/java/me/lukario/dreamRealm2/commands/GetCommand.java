...
public class GetCommand implements CommandExecutor, TabExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length == 1 && args[0].equalsIgnoreCase("Mp5")) {
            player.getInventory().addItem(Mp5.createItem());
            player.sendMessage(ChatColor.GREEN + "You have received the " + ChatColor.GOLD + "Mp5");
            return true;
        }
        if (args.length == 1 && args[0].equalsIgnoreCase("ammo_glock")) {
            for (int i = 0; i < 4; i++){
                ItemStack item = new ItemStack(Material.COPPER_INGOT);
                ItemMeta metaa = item.getItemMeta();
                metaa.setCustomModelData(6);
                metaa.setDisplayName("for_glock");
                item.setItemMeta(metaa);
